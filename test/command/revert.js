...
describe('Command::Revert', function() {
  ...
  it('should cherry pick', function() {
    ...
    commandHelper.jitCmd('revert', 'topic^');
    commandHelper.assertStdout([
      '[master a5f44a4] C',
    ].join('\n') + '\n');
  });

  it.skip('should cherry pick', function() {
    commandHelper.jitCmd('checkout', 'master');
    commandHelper.jitCmd('revert', 'topic^');
    commandHelper.jitCmd('revert', '--quit');
    commandHelper.assertStdout([
      '[master a5f44a4] C',
    ].join('\n') + '\n');
  });
  ...
  it.skip('should cherry pick', function() {
    commandHelper.jitCmd('checkout', 'master');
    commandHelper.jitCmd('revert', 'topic^');
    commandHelper.jitCmd('revert', '--abort');
    commandHelper.assertStdout([
      '[master a5f44a4] C',
    ].join('\n') + '\n');
  });
